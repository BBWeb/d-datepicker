(function() {
  ...
  module.exports = Datepicker = (function(_super) {
    ...
    Datepicker.prototype.select = function(selectedDate) {
      var currentDate, currentMonth, date, granularity, selectedMonth;
      date = moment(selectedDate.fullDate);
      selectedMonth = date.month();
      currentDate = moment(this.getCurrentDate());
      currentMonth = currentDate.month();
      granularity = this.getAttribute('granularity');
      if (selectedMonth !== currentMonth && granularity !== 'month') {
        this.gotoMonthView(date);
      }
      this.model.set("active", selectedDate.fullDate);
